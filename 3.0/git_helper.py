...
class GitRunner:
    ...
    def top_level(self):
        """Return the repository root even when the helper is launched from a subfolder."""
        ok, out = self._run(["rev-parse", "--show-toplevel"])
        if ok and out.strip():
            return True, os.path.normpath(out.strip())
        return False, out

    def normalize_to_top_level(self):
        ok, top = self.top_level()
        if ok:
            self.repo_path = top
        return ok, top
    ...
    def pull(self):
        ok, out = self._run(["pull"], timeout=180)
        ok_lfs, out_lfs = self._run(["lfs", "pull"], timeout=300)
        if ok_lfs or out_lfs:
            combined = "\n".join(part for part in [out, "git lfs pull:", out_lfs] if part)
            return ok and ok_lfs, combined
        return ok, out

    def push(self):
        ok, branch = self.current_branch()
        if ok and branch.strip() and branch.strip() != "HEAD":
            ok_up, _ = self._run(["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"])
            if not ok_up:
                return self._run(["push", "-u", "origin", branch.strip()], timeout=300)
        return self._run(["push"], timeout=300)

    def stage_all(self):
        # :/ means "from the repository root", even if the helper exe lives in a subfolder.
        return self._run(["add", "-A", ":/"])

    def stage_all_including_ignored(self):
        """
        Stage every file Git can safely add, including ignored CSV/images/exe artifacts.
        Keeps Git internals and generated caches out.
        """
        ok, raw = self._run(["ls-files", "--others", "--modified", "--deleted", "--exclude-standard", ":/"])
        ok_ignored, raw_ignored = self._run(["ls-files", "--others", "--ignored", "--exclude-standard", ":/"])
        if not ok:
            return ok, raw

        files = []
        for block in (raw, raw_ignored if ok_ignored else ""):
        ...
        for path in files:
            ...
            try:
                if os.path.isfile(abs_path) and os.path.getsize(abs_path) >= 95 * 1024 * 1024:
                    large_files.append(path.replace("\\", "/"))
            except OSError:
                pass
        if large_files:
            ok_lfs, out_lfs = self._run(["lfs", "install", "--local"], timeout=60)
            lfs_notes.append(out_lfs)
            if ok_lfs:
                for path in large_files:
                    ok_track, out_track = self._run(["lfs", "track", path], timeout=60)
                    lfs_notes.append(out_track)
                    if not ok_track:
                        return False, "Failed to track a large file with Git LFS:\n" + "\n".join(lfs_notes)
                if os.path.exists(os.path.join(self.repo_path, ".gitattributes")):
                    files.append(".gitattributes")
            else:
                return False, (
                    "One or more files are over 95 MB and should use Git LFS, but Git LFS is not available.\n"
                    + out_lfs
                )

        chunks = []
        for i in range(0, len(files), 100):
            ok_add, out_add = self._run(["add", "-f", "--"] + files[i:i + 100], timeout=120)
            chunks.append(out_add)
            if not ok_add:
                return False, "\n".join(chunks)
        notes = "\n".join(x for x in lfs_notes + chunks if x)
        return True, f"Staged {len(files)} file(s), including ignored files when needed.\n{notes}".strip()
    ...
    def status(self):
        return self._run(["status", "--porcelain=v1", "-uall"])
...
class GitHelperWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.remote_label.setFont(QFont("Consolas", 9))
        top_bar.addWidget(self.remote_label)
        top_bar.addStretch()
    ...
    def refresh_status(self):
        ...
        ok_top, top = self.git.normalize_to_top_level()
        if ok_top and os.path.normpath(self.repo_path) != os.path.normpath(top):
            self.repo_path = top
            self.setWindowTitle(f"Git Helper - {os.path.basename(self.repo_path)}")
            self.statusBar().showMessage(f"Repo: {self.repo_path}")
    ...
    def do_stage_everything(self):
        confirm = QMessageBox.question(
            self,
            "Stage Everything",
            "This stages every changed/new file in the whole repo, including ignored files.\n\n"
            "Use this for CSVs, images, PDFs, EXEs, and other files that normal Stage All skips.\n"
            "Continue?",
        )
        if confirm == QMessageBox.StandardButton.Yes:
            self._run_and_display("add -f everything", self.git.stage_all_including_ignored)
    ...
    def do_change_remote(self):
        ok_remote, current = self.git.remote_url()
        current = current.strip() if ok_remote else ""
        url, ok = QInputDialog.getText(
            self,
            "Change GitHub Repo",
            "Paste the GitHub repo URL to push/pull from:",
            text=current,
        )
    ...
    def do_choose_repo_folder(self):
        folder = QFileDialog.getExistingDirectory(
            self,
            "Choose Git Repository Folder",
            self.repo_path,
        )
        if not folder:
            return
        candidate = GitRunner(folder)
        if not candidate.is_git_repo():
            QMessageBox.warning(
                self,
                "Not a Git Repo",
                "That folder is not inside a Git repository. Put GitHelper in the project folder or initialize the folder first.",
            )
            return
        ok_top, top = candidate.top_level()
        self.repo_path = top if ok_top else folder
        self.git = GitRunner(self.repo_path)
        self.setWindowTitle(f"Git Helper - {os.path.basename(self.repo_path)}")
        self.statusBar().showMessage(f"Repo: {self.repo_path}")
        self.refresh_status()
...
def detect_or_setup_repo():
    ...
    for path in candidates:
        ...
        if runner.is_git_repo():
            ok_top, top = runner.top_level()
            return top if ok_top else path
