...
class BaseComponentItem(QGraphicsRectItem):
    ...
    def rebuild_ports(self):
        ...
        if dynamic_config:
            ...
            _simple = self.component_data.get('_simple_mode', False)
            _pos = (lambda v: v) if _simple else _snap_port_pos
            ...
            for i in range(1, count + 1):
                ...
                if side == "left":
                    x = 0
                    y = _pos(height / 2 + ((i - 1) * port_spacing - total_port_height / 2))
                elif side == "right":
                    x = width
                    y = _pos(height / 2 + ((i - 1) * port_spacing - total_port_height / 2))
                elif side == "top":
                    x = _pos(width / 2 + ((i - 1) * port_spacing - total_port_height / 2))
                    y = 0
                else:  # bottom
                    x = _pos(width / 2 + ((i - 1) * port_spacing - total_port_height / 2))
                    y = height
        ...
        if comp_type == 'Evaporator':
            ...
            # === BOTTOM HORIZONTAL HEADER (outlet manifold) ===
            # When there's no outlet distributor, draw a horizontal tube
            # connecting all outlet circuit ports at the bottom edge.
            if outlet_dist != 'Yes' and circuits > 1:
                # Remove old header line if it exists
                if hasattr(self, '_outlet_header_line') and self._outlet_header_line is not None:
                    if self._outlet_header_line.scene():
                        self._outlet_header_line.scene().removeItem(self._outlet_header_line)
                    self._outlet_header_line = None

                # Find the x positions of first and last outlet ports
                outlet_ports = [self.ports[f"outlet_circuit_{i}"] for i in range(1, circuits + 1) if f"outlet_circuit_{i}" in self.ports]
                if len(outlet_ports) >= 2:
                    from PyQt6.QtWidgets import QGraphicsLineItem
                    x_first = outlet_ports[0].pos().x()
                    x_last = outlet_ports[-1].pos().x()
                    y_line = height  # at the bottom edge, same y as ports
                    line = QGraphicsLineItem(x_first, y_line, x_last, y_line, self)
                    line.setPen(QPen(QColor("#000000"), 3))
                    self._outlet_header_line = line

            # === TOP HORIZONTAL HEADER (inlet manifold) ===
            # When there's no inlet distributor, draw a horizontal tube
            # connecting all inlet circuit ports at the top edge.
            if inlet_dist != 'Yes' and circuits > 1:
                # Remove old header line if it exists
                if hasattr(self, '_inlet_header_line') and self._inlet_header_line is not None:
                    if self._inlet_header_line.scene():
                        self._inlet_header_line.scene().removeItem(self._inlet_header_line)
                    self._inlet_header_line = None

                # Find the x positions of first and last inlet ports
                inlet_ports = [self.ports[f"inlet_circuit_{i}"] for i in range(1, circuits + 1) if f"inlet_circuit_{i}" in self.ports]
                if len(inlet_ports) >= 2:
                    from PyQt6.QtWidgets import QGraphicsLineItem
                    x_first = inlet_ports[0].pos().x()
                    x_last = inlet_ports[-1].pos().x()
                    y_line = 0  # at the top edge, same y as ports
                    line = QGraphicsLineItem(x_first, y_line, x_last, y_line, self)
                    line.setPen(QPen(QColor("#000000"), 3))
                    self._inlet_header_line = line
...
class PipeItem(QGraphicsPathItem):
    ...
    def _set_path_from(self, pts):
        ...
        for is_start in (True, False):
            ...
            if not (comp and (comp.component_data or {}).get('type') == 'Junction'):
                continue
            if (comp.component_data or {}).get('_simple_mode'):
                continue
    ...
    def update_path(self):
        """Re-pin the route to the ports and redraw it. Generated diagrams
        with `_raw_route=True` bypass the entire pipeline (no sanitize, no
        port pin, no junction trim) — what you write is what you see."""
        if self.pipe_data.get('_raw_route'):
